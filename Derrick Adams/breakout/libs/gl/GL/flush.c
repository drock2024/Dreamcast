...
static int PT_ENABLED = 1;
void glKosSwapBuffers(void);
void glKosSwapBuffers_NO_PT(void);
...
void APIENTRY glKosInitEx(GLdcConfig* config) {
    ...
    PT_ENABLED = !!(config->initial_pt_capacity);
...
void APIENTRY glKosSwapBuffers_NO_PT() {
    pvr_wait_ready();
