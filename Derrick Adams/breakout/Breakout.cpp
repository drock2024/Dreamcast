...
void Breakout::drawBackground(void) {
    //glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); //Unsupported under DC
    glBegin(GL_QUADS);
    // Top color
...
void Breakout::drawBalls(void) {
    for (std::vector<Ball>::iterator it = balls.begin(); it != balls.end(); ) {
        //glPolygonMode(GL_FRONT_AND_BACK, GL_FILL); // use GL_LINE if no fill //Unsupported under DC
        glBegin(GL_POLYGON);
...
void Breakout::drawScore(void) {
    //Remove text rendering for now, would need to be fully rewritten
...
void Breakout::mouseClick(int button, int state, int x, int y) {
    //Input handling will be done in a later step 
