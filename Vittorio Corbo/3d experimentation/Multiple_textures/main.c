...
int main(int argc, char **argv) {
   ...
   // glClearDepth(1.0f);
    ...
    //texture[0] = glTextureLoadPVR("/rd/newbricks.pvr", 0, 0);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);

    //cobbles
	//texture[1] = glTextureLoadPVR("/rd/cobbles.pvr", 0, 0);	
	//changing to see if problem is placing or what
	texture[1] = glTextureLoadPVR("/rd/walls.pvr", 0, 0);
	//trying this filter, see if it shows it normally
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
